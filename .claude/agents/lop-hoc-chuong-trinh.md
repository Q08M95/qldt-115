...
- CRUD bài giảng gắn với lớp (tên, chuyên đề, **số tiết cho phép < 1** — vd 0.5, kiểu `numeric`), tự do sửa/xoá riêng từng lớp. Mỗi bài có thêm: dropdown gán vào 1 buổi (tuỳ chọn, "Chưa gom buổi" mặc định), cờ `mo_dang_ky` riêng, 2 trường chỉ định riêng — dùng khi cần đăng ký/chỉ định ở granularity từng bài thay vì cả buổi/lớp. Thứ tự sắp xếp bằng **kéo-thả** (`@dnd-kit`), lưu qua RPC `reorder_bai_giang` — không có ô nhập số thứ tự thủ công, bài mới luôn thêm cuối danh sách.
