...
**Đã xây (2026-09-14)**: `DangKyDialog` (thẻ danh sách chỉ "cả lớp"; trang chi tiết đủ cả lớp/buổi/bài đang `mo_dang_ky`), `DangKyList` (bảng + `Sheet` xem hồ sơ trước khi duyệt) trong tab "Đăng ký & Duyệt", RPC `duyet_dang_ky`/`tu_choi_dang_ky`/`dang_ky_phu_hop_nhom`, trigger `notify_dang_ky_moi`. **Cố ý để lại cho Giai đoạn 6**: `lich_giang` tạo ra ở bước duyệt CHƯA có `ngay_gio`/`buoi`/`dia_diem` — xếp lịch cụ thể là việc của agent này ở Giai đoạn 6 (`/lich-giang`), Giai đoạn 5 chỉ xác nhận nhân sự. **Chưa xây (cân nhắc thêm nếu người dùng yêu cầu)**: nút "huỷ đăng ký" tự phục vụ cho giảng viên/trợ giảng (RLS `dang_ky_giang_day_delete` hiện chỉ cho `is_admin()`, chưa cho tự xoá đăng ký `cho_duyet` của chính mình — cần hỏi người dùng trước khi mở rộng policy này).
