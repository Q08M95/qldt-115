...
> - **Tạo lớp thành luồng 2 bước** (`/lop-hoc/moi`, thay hẳn dialog "Thêm lớp học" cũ — đã xoá `add-class-dialog.tsx`): Bước 1 là trang riêng (`TaoLopForm`, y hệt nội dung dialog cũ + thanh chỉ báo bước) tạo xong tự điều hướng sang `/lop-hoc/{id}?wizard=step2`; Bước 2 tái dùng nguyên trang chi tiết lớp + tab "Bài giảng" đã có sẵn (không viết lại), chỉ thêm `WizardBanner` ở đầu trang nhắc "Bước 2/2" + nút "Hoàn tất thiết lập" (bấm là bỏ query param, không khoá/ẩn gì, vẫn sửa lại bình thường sau đó).
> - **Hộp thư "Cần duyệt" tổng hợp mọi lớp** (`ChoDuyetPanel`, đặt đầu trang `/lop-hoc`, chỉ admin/quản lý thấy): trước đó muốn duyệt phải tự vào từng lớp một để kiểm tra, giờ thấy toàn bộ đăng ký chờ duyệt của mọi lớp ngay khi vào `/lop-hoc`, Duyệt/Từ chối/Huỷ ngay tại đó, không cần đổi cấu trúc quyền (RLS `dang_ky_giang_day_select` vốn đã cho `is_quan_ly()` xem hết). **Cố ý đặt ở `/lop-hoc` chứ không phải `/dashboard`** dù đó là nơi CLAUDE.md mục 3 gợi ý ban đầu — tránh lấn phạm vi Giai đoạn 9 (`dashboard-bao-cao`) khi trang `/dashboard` hiện vẫn là stub cố tình chưa mở rộng nghiệp vụ.
