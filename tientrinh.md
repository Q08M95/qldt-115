...
> - **Sửa đổi 2026-09-14 (tiếp), theo yêu cầu người dùng**: `dang_ky_giang_day_delete` mở thêm điều kiện tự huỷ — `profile_id = auth.uid() and trang_thai = 'cho_duyet'` (trước đó chỉ `is_admin()`). Giảng viên/trợ giảng tự huỷ được đăng ký của chính mình khi còn chờ duyệt; đã `da_duyet` thì không tự xoá được nữa vì đã có `lich_giang` tương ứng — muốn rút lui sau khi đã duyệt phải nhờ quản lý xử lý qua lịch giảng ở Giai đoạn 6.

> **Cải thiện luồng thao tác 2026-09-14 (không đổi schema), theo phản hồi người dùng**: sau khi test, người dùng nhận xét thao tác liên quan lớp học (tạo lớp, đăng ký, duyệt) bị "nhảy qua nhảy lại" giữa nhiều dialog/tab rời rạc, muốn 1 luồng liền mạch dễ quan sát đang ở bước nào. 2 thay đổi:
> - Tạo lớp thành luồng 2 bước (`/lop-hoc/moi` + `WizardBanner`) — **đã bị thay thế hoàn toàn cùng ngày, xem note "Thiết kế lại canvas 1 trang" ngay dưới đây**, không còn tồn tại trong code.
> - **Hộp thư "Cần duyệt" tổng hợp mọi lớp** (`ChoDuyetPanel`, đặt đầu trang `/lop-hoc`, chỉ admin/quản lý thấy) — mục này vẫn giữ nguyên, không đổi: trước đó muốn duyệt phải tự vào từng lớp một để kiểm tra, giờ thấy toàn bộ đăng ký chờ duyệt của mọi lớp ngay khi vào `/lop-hoc`, Duyệt/Từ chối/Huỷ ngay tại đó, không cần đổi cấu trúc quyền (RLS `dang_ky_giang_day_select` vốn đã cho `is_quan_ly()` xem hết). **Cố ý đặt ở `/lop-hoc` chứ không phải `/dashboard`** dù đó là nơi CLAUDE.md mục 3 gợi ý ban đầu — tránh lấn phạm vi Giai đoạn 9 (`dashboard-bao-cao`) khi trang `/dashboard` hiện vẫn là stub cố tình chưa mở rộng nghiệp vụ.

> **Thiết kế lại canvas 1 trang 2026-09-14 (không đổi schema), theo yêu cầu người dùng "thiết kế lại toàn bộ cấu trúc — số thao tác đơn giản nhất")**: người dùng đánh giá bản wizard 2 bước ở trên (mới làm cùng ngày) vẫn còn quá nhiều điều hướng/dialog rời rạc, chọn phương án mạnh tay hơn qua `AskUserQuestion` ("Canvas 1 trang, tự lưu"). Kiến trúc mới của `/lop-hoc/[id]`:
> - **Bỏ hẳn `Tabs`** ("Bài giảng" / "Đăng ký & Duyệt" / "Lịch giảng") — 3 khối nay là các `<section>` xếp dọc trên cùng 1 trang, cuộn xuống thấy hết, không bấm chuyển tab.
> - **Bỏ hẳn wizard 2 trang** (`/lop-hoc/moi`, `TaoLopForm`, `WizardBanner` — cả 3 đã xoá file). Tạo lớp giờ là 1 dialog rút gọn `QuickCreateClassDialog` chỉ hỏi **Tên lớp** (bắt buộc) + **Chương trình mẫu** (tuỳ chọn — giữ lại vì CLAUDE.md mục 4 bắt buộc copy bài giảng từ template ngay lúc tạo, không thêm lại được sau). Mọi trường khác dùng giá trị mặc định hợp lệ (so_giang_vien_can/so_tro_giang_can = 1, co_kinh_phi = true) và sửa ngay tại canvas sau khi tạo — tạo xong `router.push` thẳng tới `/lop-hoc/{id}`, không còn query param `?wizard=step2`.
> - **Bỏ hẳn `EditClassDialog`** (đã xoá file) — thông tin lớp (toàn bộ trường của `ClassFormFields`, gồm cả chỉ định GV/TG) sửa **trực tiếp tại chỗ** trên canvas qua `LopInfoAutosaveForm`, **tự lưu (autosave)**: input dạng chữ/ngày/số lưu khi rời trường (`onBlur`), Select/checkbox lưu ngay khi đổi (`onValueChange`/`onChange`) — cả 2 loại đều gom qua 1 debounce 500ms rồi đọc lại toàn bộ `FormData` của form và gọi lại nguyên `updateLopHoc` đã có (không tạo action mới, không tạo RPC mới). `ClassFormFields` (dùng chung) nhận thêm 1 prop tuỳ chọn `onDirty` để phát tín hiệu này — `QuickCreateClassDialog` không dùng `ClassFormFields` nữa nên không bị ảnh hưởng.
> - **Buổi giảng & Bài giảng — thêm mới không còn dialog**: `BuoiGiangQuickAdd`/`BaiGiangQuickAdd` là 1 dòng input luôn hiện sẵn (nhập tên rồi Enter/bấm Thêm), tạo ngay với giá trị mặc định hợp lý (GV/TG cần = 1/1, số tiết = 1). `BuoiGiangDialog`/`BaiGiangDialog` cũ được **đơn giản hoá thành chỉ còn chế độ Sửa** (bỏ hẳn nhánh "create", prop bắt buộc thay vì tuỳ chọn) — sửa chi tiết (số lượng cần, chỉ định, mở đăng ký, chuyên đề...) vẫn qua dialog này vì tần suất sửa thấp hơn nhiều so với thêm mới, không đáng để inline hoá luôn.
...
> **Bảng phân công (roster board) 2026-09-14 (không đổi schema), theo yêu cầu người dùng**: người dùng mô tả rõ mô hình mong muốn — GV/TG thấy đúng cấu trúc buổi/bài của lớp đang mở đăng ký để chọn dạy, đăng ký tự do trong nhóm phù hợp (không giới hạn số lượng), quản lý xem hết mọi lượt để duyệt, "số lượng" chỉ là con số tham khảo. Thay `DangKyDialog` (dropdown gộp lớp/buổi/bài) + `DangKyList` (bảng phẳng) tại `/lop-hoc/[id]` bằng **`DangKyRosterBoard`** (`dang-ky-roster-board.tsx`) — cây "Cả lớp" → từng buổi ("Cả buổi" + các bài thuộc buổi) → "Bài chưa gom buổi", mỗi dòng chỉ hiện khi `mo_dang_ky` hoặc đã có đăng ký (tránh liệt kê rỗng vô nghĩa). Không action/RPC mới — dùng lại nguyên `createDangKy`/`duyetDangKy`/`tuChoiDangKy`/`huyDangKy` đã có, chỉ tổ chức lại UI:
> - **Quản lý** thấy avatar chip từng người đăng ký ở đúng dòng buổi/bài, bấm vào mở `Sheet` xem hồ sơ + Duyệt/Từ chối (tái dùng `TuChoiDialog`). Badge "X/Y đã duyệt" ở đầu (toàn lớp) và mỗi buổi tính bằng `COUNT DISTINCT profile_id` đã `da_duyet` (1 người đăng ký nhiều bài trong cùng buổi chỉ tính 1) — `so_giang_vien_can`/`so_tro_giang_can` giữ nguyên vai trò **mục tiêu tham khảo**, không phải giới hạn cứng.
...
3. Form tạo lớp học (từ 2026-09-14, phiên bản canvas — xem note "Thiết kế lại canvas 1 trang" cuối Giai đoạn 5: dialog rút gọn `QuickCreateClassDialog` chỉ hỏi tên lớp + chương trình mẫu, mọi trường khác sửa ngay sau khi tạo tại trang chi tiết): chọn **chương trình mẫu (tuỳ chọn)** — nếu chọn, hệ thống tự động **copy** toàn bộ `chuong_trinh_mau_bai_giang` thành các dòng `bai_giang` của riêng lớp này; nếu không chọn, tạo lớp trống rồi thêm bài giảng thủ công. Trường khác: loại lớp, đối tượng học viên, tính chất lớp (kèm hiển thị **kinh phí** ở cả trang chi tiết, không chỉ thẻ danh sách), số GV/TG cần, cờ **mở đăng ký**, **nhóm giảng viên/trợ giảng phù hợp** (chọn nhiều trong 5 nhóm phân loại của `profiles`). **Chỉ định giảng viên/trợ giảng**: UI tự sinh **đúng N khung chọn người theo số GV/TG cần** (cần 1 hiện 1 khung, cần 2 hiện 2 khung..., cập nhật ngay khi đổi số lượng cần trong form) — danh sách chọn ở mỗi khung **lọc theo nhóm phù hợp đã chọn** (nếu có chọn nhóm), không cho chọn trùng 1 người ở 2 khung khác nhau.
4. Trang chi tiết `/lop-hoc/[id]`: từ 2026-09-14 là **canvas 1 trang cuộn dọc** (không còn tab con — xem note "Thiết kế lại canvas 1 trang" cuối Giai đoạn 5), gồm các `<section>` xếp liên tiếp: Thông tin lớp (tự lưu), Bài giảng (gồm **Buổi giảng** + **Bài giảng**) hoàn thiện ở giai đoạn này; khung "Đăng ký & Duyệt" và "Lịch giảng" scaffold sẵn để Giai đoạn 5-6 lắp nội dung thực vào cùng vị trí (xem mục 7 ranh giới bên dưới).
