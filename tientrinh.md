...
2. Trang `/lop-hoc` (thuộc nhóm sidebar "Đào tạo"): **dạng thẻ (card), bố cục 3 cột kiểu Kanban** — mỗi cột là 1 trong 3 trạng thái (Chưa mở / Đang diễn ra / Hoàn thành, xếp cạnh nhau trên desktop, xếp chồng trên mobile), không dùng bảng + filter trạng thái nữa vì đã nhóm sẵn theo cột. Bộ lọc: tìm theo tên (debounce), toggle nhiều lựa chọn theo **loại lớp** (`LOAI_LOP_VALUES` cố định: ABCDE, ACLS, BLS, SCC-CĐ, BTXH, SCC-LX — khớp đúng 6 chương trình đào tạo thật, `loai_lop` đổi từ nhập tự do sang Select cố định trong form), và theo đối tượng học viên. Mỗi thẻ hiện: tên lớp, loại lớp (badge), đối tượng, tính chất lớp, ngày khai giảng-kết thúc, badge "Mở đăng ký" nếu `mo_dang_ky`, người được chỉ định nếu có.
