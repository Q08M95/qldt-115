...
2. Trang `/lop-hoc` (thuộc nhóm sidebar "Đào tạo"): **dạng thẻ (card), nhóm theo 3 trạng thái** (Chưa mở / Đang diễn ra / Hoàn thành) — trên desktop xếp 3 cột cạnh nhau theo thứ tự **Đang diễn ra → Chưa mở → Hoàn thành**, mỗi cột tự cuộn riêng (`max-h-[75vh]`) để trang không bị kéo dài quá mức khi 1 nhóm tích luỹ nhiều lớp; trên mobile chuyển thành **Tab ngang** cùng thứ tự, chỉ hiện 1 nhóm tại 1 thời điểm thay vì xếp dọc cả 3 nhóm nối tiếp (sửa 2026-09-10 theo yêu cầu người dùng, xem thẻ card chi tiết bên dưới). Bộ lọc gọn trên 1 hàng: ô tìm theo tên (debounce), dropdown checkbox chọn nhiều **loại lớp** (`LOAI_LOP_VALUES` cố định: ABCDE, ACLS, BLS, SCC-CĐ, BTXH, SCC-LX — thay cho dải chip dễ rớt dòng), nút lọc nâng cao (icon phễu) chứa lựa chọn nhanh **đối tượng học viên**.
   - **Thẻ lớp (Course Card)**: có dải ảnh bìa (thumbnail) cao ~64px ở đầu thẻ — màu dải tái sử dụng đúng 6 tông "data-*" đã kiểm chứng OKLCH/CVD ở CLAUDE.md mục 3 (gán cố định 1-1 theo `loai_lop`, xem `LOAI_LOP_BAND_CLASS` trong `class-card.tsx`; 6 tông này vốn gắn vai trò dữ liệu khác — giảng viên/trợ giảng... — nhưng trang này không hiển thị màu theo vai trò người nên tái dùng cho chiều dữ liệu khác không xung đột), kèm icon minh hoạ + nhãn loại lớp dạng chữ (không dùng màu làm kênh phân biệt duy nhất). Góc trái dải hiện pill badge trạng thái, góc phải hiện pill "Mở đăng ký" nếu `mo_dang_ky`. Thân thẻ: tên lớp, badge đối tượng/tính chất lớp, dòng icon+chữ cho ngày khai giảng-kết thúc, chỉ tiêu GV/TG cần và người được chỉ định (nếu có). Cuối thẻ có nút CTA màu primary chạy suốt chiều rộng, nhãn đổi theo `mo_dang_ky` ("Đăng ký ngay"/"Xem chi tiết") — hiện tại cả 2 đều dẫn vào trang chi tiết lớp vì nút đăng ký thật thuộc Giai đoạn 5, tránh dựng nút giả không có hành vi.
