...
>
> Lịch sử rút gọn các đợt trước (2026-09-14, đã hết hiệu lực về mặt bề mặt nhưng các quyết định nghiệp vụ/hành vi vẫn giữ nguyên): Đợt 1-3 dựng khung sườn kính mờ ban đầu + chuẩn hoá `strokeWidth={1.5}` cho icon + tạo `Checkbox` dùng chung thay input mặc định trình duyệt (2 việc này vẫn giữ nguyên, không bị ảnh hưởng bởi đổi bề mặt). Đợt 4-5 chỉnh nhãn/tooltip/header (phần lớn bị ghi đè bởi đợt chốt lại này). Đợt 6 (2026-09-15) — khung app đổi sang **chiếm trọn viewport, không viền/bo góc/gradient ngoài** (người dùng xác nhận đây là web app toàn màn hình, không phải cửa sổ nổi kiểu desktop) — `--app-gradient` từ nay chỉ còn dùng ở trang auth. Đợt 7 (2026-09-15) — redesign mẫu module Nhân sự theo đúng cấu trúc `mauthietke.png` (trang chi tiết `/nhan-su/[id]`): bỏ `Tabs` "Hồ sơ/Chứng chỉ" cũ, thay bằng khối đầu trang (avatar + tên + badge + dòng metadata chức danh/khoa phòng, giống khối "Dunder Mifflin") và bố cục 2 cột — trái là card "Thông tin cá nhân" tint màu theo vai trò (giống "About Company"), phải là panel "Chứng chỉ" dạng danh sách dọc gọn (icon + tiêu đề + dòng meta + badge trạng thái, không còn là `Table` nhiều cột — không hợp trong cột hẹp). Đây là mẫu tham khảo cho các module khác khi redesign theo cùng phong cách. Đợt 8 (2026-09-15) — áp dụng tiếp sang trang danh sách `/nhan-su` theo yêu cầu người dùng: đổi thành **pattern master-detail 2/3 + 1/3 trên cùng 1 trang** (không còn trang danh sách thuần) — cột trái 2/3 là bảng rút gọn chỉ còn Họ tên/Vai trò/Nhóm/Trạng thái (Học vị/Chuyên môn/Email bỏ khỏi bảng, chỉ xem trong panel chi tiết); bấm vào 1 dòng set query param `?xem=<id>` (không dùng client state, giữ URL chia sẻ được/nút Back hoạt động đúng) để cột phải 1/3 hiện **xem chi tiết đầy đủ, read-only** (tái dùng khối đầu trang + card tint màu + `CertificateList` của `/nhan-su/[id]`) kèm nút "Sửa hồ sơ" dẫn sang trang `/nhan-su/[id]` đầy đủ để sửa thật — **không sửa trực tiếp trong panel hẹp** (quyết định người dùng: panel 1/3 luôn read-only). 3 bộ lọc cũ (Vai trò/Trạng thái/Nhóm phân loại) **bỏ hẳn** theo yêu cầu người dùng ("không cần thiết nữa"), `NhanSuFilters` chỉ còn 1 ô tìm kiếm theo tên. Responsive: dưới `lg` chỉ hiện 1 trong 2 cột tại 1 thời điểm (danh sách hoặc panel chi tiết full-width kèm nút "Quay lại danh sách"), thay vì hiện cả 2 cột chật — làm bằng CSS thuần (`hidden lg:flex` có điều kiện theo `xem` có giá trị hay không), không cần JS/client state.

> **Đợt 9 (2026-09-15)** — đảo ngược tỉ lệ 2 cột của Đợt 8 theo yêu cầu người dùng, bám sát đúng tỉ lệ thật trong `mauthietke.png` (vùng chi tiết đối tượng chiếm phần lớn, danh sách chỉ là dải phụ hẹp): **cột trái 2/3 giờ là vùng chi tiết 1 nhân sự** (khối đầu trang không đổi + `Tabs` 2 cấp — "Tổng quan"/"Chứng chỉ" — mỗi tab là 1 card tint màu riêng theo loại nội dung: "Tổng quan" tint theo màu vai trò như trước, "Chứng chỉ" tint trung tính `bg-muted/40` vì không có màu vai trò dữ liệu nào hợp cho "tài liệu"); **cột phải 1/3 giờ là danh sách chung gọn tối đa** — chỉ avatar nhỏ + tên + vai trò (không còn `Table`, không còn cột Nhóm/Trạng thái riêng, chỉ hiện badge "Đã khoá" nếu có). **Tìm kiếm tách hẳn thành 1 hàng riêng** phía trên toàn bộ vùng 2 cột (không nằm trong cột nào), giống thanh search nổi bật đầu `mauthietke.png`. **Mặc định tự chọn người đầu tiên trong danh sách** khi chưa bấm `?xem=` — tránh vùng chính 2/3 trống lúc mới vào trang (đảo ngược cách xử lý "trạng thái rỗng" của Đợt 8). `PAGE_SIZE` tăng lên 30 vì mỗi dòng trong cột 1/3 giờ rất thấp (chỉ avatar+tên).
...
- **Khung cửa sổ hợp nhất** (`(app)/layout.tsx`, áp dụng đồng bộ cả desktop lẫn mobile — xem mục 3.2):
  ...
  - **Khung cửa sổ chiếm TRỌN viewport** (chốt lại 2026-09-15, đảo ngược quyết định "nổi có viền/gradient/bo góc ngoài" ban đầu ngay trong cùng ngày) — người dùng xác nhận đây không phải app nổi kiểu cửa sổ desktop mà là 1 web app chiếm toàn màn hình: bỏ hẳn `p-4`/`app-gradient-bg`/`rounded-[24px]`/`border`/`shadow-xl` ở wrapper ngoài cùng trong `(app)/layout.tsx`. Sidebar + nội dung áp sát mép trình duyệt, không viền, không bo góc ngoài, không nền gradient phía sau.
- **Hình khối & cấu trúc**: bo góc **vừa phải** — `rounded-2xl` (~18px) cho `Card`/panel/dialog/menu con bên trong nội dung. Khung cửa sổ ngoài cùng KHÔNG bo góc (chiếm trọn viewport, xem trên). `Button`, `Badge`, `Tabs` vẫn dạng **viên thuốc** (`rounded-full`) — phần này giữ nguyên vì đã khớp `mauthietke.png`.
- **Bề mặt — nền đặc, KHÔNG dùng kính mờ** (xem chi tiết mục 3.1) — áp dụng cho MỌI khối, không có ngoại lệ.
- **Nền gradient trừu tượng mềm** (`--app-gradient` trong `globals.css`): **không còn dùng ở khung app chính** (đã bỏ theo quyết định "chiếm trọn viewport" ở trên) — chỉ còn dùng ở các trang auth (`/login`, `/register`...) làm nền phía sau card đăng nhập, nơi không có khung cửa sổ sidebar/nội dung.
